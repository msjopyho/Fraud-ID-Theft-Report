...
def display_map(df, year, quarter):
    ...
    for feature in choropleth.geojson.data['features']:
        ...
        feature['properties']['population'] = 'Population: ' + '{:,}'.format(
            df_indexed.loc[state_name, 'State Pop'][0]) if state_name in list(df_indexed.index) else ''
        feature['properties']['per_100k'] = 'Reports/100K Population: ' + str(
            round(df_indexed.loc[state_name, 'Reports per 100K-F&O together'][0])) if state_name in list(
            df_indexed.index) else ''
...
def display_loss_facts(df, year, quarter, report_type, state_name, field, title, string_format='${:,}',
                       is_median=False):
    df = df[(df['Year'] == year) & (df['Quarter'] == quarter)]
    df = df[df['Report Type'] == report_type]
    if state_name:
...
def main():
    ...
    with col1:
        display_fraud_facts(df_fraud, year, quarter, report_type, state_name, 'State Fraud/Other Count',
                            f'Number of {report_type} Reports')
    with col2:
        display_loss_facts(df_median, year, quarter, report_type, state_name, 'Overall Median Losses Qtr',
                           'Median $ Loss', is_median=True)
    with col3:
        display_loss_facts(df_loss, year, quarter, report_type, state_name, 'Total Losses', 'Total $ Loss')
